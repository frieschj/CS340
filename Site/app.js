...
// Date: 11/30/2023
// Adapted from GitHub Node.js starter app walkthrough guide
// Changed the route and changed the page being rendered.
// Source URL: https://github.com/osu-cs340-ecampus/nodejs-starter-app/tree/main/Step%203%20-%20Integrating%20a%20Templating%20Engine%20(Handlebars)

// Operations on Trails entity not yet implemented - this route displays a static page.
...
// Route handles DELETE functionality for the Trails entity.
...
app.put('/put-trail-ajax', function(req,res,next){
    ...
    let queryUpdateTrailEnvironment = `UPDATE Trails SET parkID = ?, elevation = ?, difficultyID = ?, length = ?, avgTime = ?, avalancheRisk = ? WHERE Trails.trailID = ?`;
    let selectPark = `SELECT * FROM Parks WHERE parkID = ?`
    let selectDifficulty = `SELECT * FROM TrailDifficulties WHERE difficultyID = ?`
  
          // Run the 1st query
          db.pool.query(queryUpdateTrailEnvironment, [park, elevation, difficulty, length, avgTime, avalancheRisk, trail], function(error, rows, fields){
              if (error) {
...
app.get('/TrailDifficulties', function(req, res){  
        ...
        db.pool.query(query1, function(error, rows, fields){    // Execute the query
            ...
            res.render('TrailDifficulties', {data: rows});                  // Render the index.hbs file, and also send the renderer
